	...
	for _, tc := range tests {
		t.Run(tc.Name, func(t *testing.T) {
			...
			if tc.Err != nil {
				...
				// configmap checks
				fakeCM, err := fakeClient.CoreV1().ConfigMaps(testViper.GetString("namespace")).Get(context.TODO(), mittensConfigMapPrefix+fakeDeployment.Name, metav1.GetOptions{})
				require.Nil(err)
				require.NotNil(fakeCM)
				require.True(strings.Contains(fakeCM.Name, mittensConfigMapPrefix))
				...
				Spec: v1.PodSpec{
					Containers: []v1.Container{
						...
						},
						{
							...
							Image: "ghcr.io/lappihuan/mittens-mitmproxy:latest",
						},
					},
					Volumes: []v1.Volume{
						{
							Name: mittensConfigMapPrefix + "sample-deployment",
						},
					},
				},
			},
...
func fakeClientUntappedNamedPortsMissing() *fake.Clientset {
	...
	var ports []v1.ServicePort
	for _, p := range service.Spec.Ports {
		p.TargetPort = intstr.FromString(mittensPortName)
		ports = append(ports, p)
	}
	service.Spec.Ports = ports
	return fake.NewSimpleClientset(
